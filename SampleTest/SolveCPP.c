...
#else
  #define CASADI_PREFIX(ID) SolveCPP_ ## ID
#endif
...
/* nlp_grad:(x[3],p[2],lam_f,lam_g[2])->(f,g[2],grad_gamma_x[3],grad_gamma_p[2]) */
...
/* nlp_grad_f:(x[3],p[2])->(f,grad_f_x[3]) */
...
/* nlp_jac_g:(x[3],p[2])->(g[2],jac_g_x[2x3]) */
...
/* nlp_hess_l:(x[3],p[2],lam_f,lam_g[2])->(triu_hess_gamma_x_x[3x3,3nz]) */
...
/* solver:(x0[3],p[2],lbx[3],ubx[3],lbg[2],ubg[2],lam_x0[3],lam_g0[2])->(x[3],f,g[2],lam_x[3],lam_g[2],lam_p[2]) */
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  if (d_nlp.x0) {
    casadi_copy(d_nlp.x0, 3, d_nlp.z);
  } else {
    casadi_clear(d_nlp.z, 3);
  }
  if (d_nlp.lbx) {
    casadi_copy(d_nlp.lbx, 3, d_nlp.lbz);
  } else {
    casadi_fill(d_nlp.lbz, 3, -casadi_inf);
  }
  if (d_nlp.ubx) {
    casadi_copy(d_nlp.ubx, 3, d_nlp.ubz);
  } else {
    casadi_fill(d_nlp.ubz, 3, casadi_inf);
  }
  if (d_nlp.lam_x0) {
    casadi_copy(d_nlp.lam_x0, 3, d_nlp.lam);
  } else {
    casadi_clear(d_nlp.lam, 3);
  }
  if (d_nlp.lbg) {
    casadi_copy(d_nlp.lbg, 2, d_nlp.lbz+3);
  } else {
    casadi_fill(d_nlp.lbz+3, 2, -casadi_inf);
  }
  if (d_nlp.ubg) {
    casadi_copy(d_nlp.ubg, 2, d_nlp.ubz+3);
  } else {
    casadi_fill(d_nlp.ubz+3, 2, casadi_inf);
  }
  if (d_nlp.lam_g0) {
    casadi_copy(d_nlp.lam_g0, 2, d_nlp.lam+3);
  } else {
    casadi_clear(d_nlp.lam+3, 2);
  }
  d->nlp = &d_nlp;
  d->prob = &p;
  p.nlp = &p_nlp;
  ...
  AddIpoptStrOption(d->ipopt, (char *)"linear_solver", (char*)"mumps");
  ...
  casadi_copy(d_nlp.z, 3, d_nlp.x);
  casadi_copy(d_nlp.z + 3, 2, d_nlp.g);
  casadi_copy(d_nlp.lam, 3, d_nlp.lam_x);
  casadi_copy(d_nlp.lam + 3, 2, d_nlp.lam_g);
  if (d_nlp.f) casadi_copy(&d_nlp.objective, 1, d_nlp.f);
  if (d_nlp.lam_p) casadi_copy(d_nlp.lam_p, 2, d_nlp.lam_p);
  return 0;
  return 0;
}
...
int Run(const double ** arg, double **res) {
  __int64 iw[100];
  double w[100];
  return solver(arg, res, iw, w, 0);
}
