...
extern "C" int Run(const double** arg, double** res);
...
int TestSample() {
  ...
  DMVector argdm = solver.get()->convert_arg(arg);
  int dm_size = 0;
  arg_list.resize(argdm.size());
  for (int i = 0; i < argdm.size(); i++) {
    const Matrix<double>& dm0 = argdm[i];
    int cols = dm0.columns();
    int rows = dm0.rows();
    dm_size += cols * rows;
  }
  args_vec.resize(dm_size);
  int CurrentIdx = 0;
  for (int i = 0; i < argdm.size(); i++) {
    const Matrix<double>& dm0 = argdm[i];
    arg_list[i] = &args_vec[CurrentIdx];
    memcpy(&args_vec[CurrentIdx], dm0->data(), dm0->size() * sizeof(double));
    CurrentIdx += dm0->size();
  }
  ...
  std::vector<double> res_test_vec;
  DMVector resDM = solver->convert_res(res);
  dm_size = 0;

  res_list.resize(resDM.size());
  for (int i = 0; i < resDM.size(); i++) {
    const Matrix<double>& dm0 = resDM[i];
    int cols = dm0.columns();
    int rows = dm0.rows();
    dm_size += cols * rows;
  }

  res_vec.resize(dm_size);
  res_test_vec.resize(dm_size);
  CurrentIdx = 0;
  for (int i = 0; i < resDM.size(); i++) {
    const Matrix<double>& dm0 = resDM[i];
    res_list[i] = &res_vec[CurrentIdx];
    memset(&res_vec[CurrentIdx], 0, dm0->size() * sizeof(double));
    memcpy(&res_test_vec[CurrentIdx], dm0->data(), dm0->size() * sizeof(double));
    CurrentIdx += dm0->size();
  }

  int re = Run((const double**)&arg_list[0], &res_list[0]);
